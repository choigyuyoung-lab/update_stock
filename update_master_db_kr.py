...
# 4. 페이지 처리 로직 (미국 지수 추종 ETF 판별 기능 추가)
def process_page_kr(page, engine, client, config):
    ...
    if item is not None:
        ...
        # 🌟 미국 지수 추종 ETF 판별 로직[cite: 3, 4]
        us_tracking_tag = None
        target_m_t = None
        
        if is_etf:
            if "S&P500" in stock_name:
                target_m_t = "SPY"
                us_tracking_tag = "S&P 500"
            elif any(x in stock_name for x in ["나스닥100", "NASDAQ100"]):
                target_m_t = "QQQ"
                us_tracking_tag = "NASDAQ 100"
            elif "미국" in stock_name and "반도체" in stock_name:
                # 필라델피아 반도체 등은 SOXX로 연결 가능
                target_m_t = "SOXX"
                us_tracking_tag = "US Semiconductor"
            
        # 기존 시장BM 로직 (미국 지수가 아닐 때만 적용)[cite: 3]
        if not target_m_t:
            if clean_t in engine.k200_list: target_m_t = "069500"
            elif clean_t in engine.kd150_list: target_m_t = "229200"
            elif is_etf: target_m_t = "292190" # 일반 ETF는 KODEX 300
