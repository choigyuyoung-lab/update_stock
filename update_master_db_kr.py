...
FORCE_UPDATE = True # 강제 업데이트 플래그

logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')
logger = logging.getLogger(__name__)

# ---------------------------------------------------------
# 2. 지표 DB 분석 (KR시장, KR산업 네이밍 반영)
# ---------------------------------------------------------
def get_dynamic_config(client):
    """지표 DB의 구분값을 읽어 KR시장/KR산업 지표를 자동 로드"""
    logger.info("🔍 지표지수 DB 분석 중 (KR/US 구분 체계 적용)...")
    config = {"ticker_to_id": {}, "kr_industry_tickers": [], "us_industry_tickers": []}
    ...
    try:
        ...
        for page in pages:
            ...
            if category == "KR산업":
                ...
                config["us_industry_tickers"].append(ticker)
...
# ---------------------------------------------------------
# 3. 데이터 엔진 (Python 3.10+ 기준)[cite: 1]
# ---------------------------------------------------------
class StockAutomationEngine:
    def __init__(self, kr_industry_tickers):
        logger.info("📡 KRX 데이터 엔진 가동 (requests + StringIO 사용)[cite: 1]")
        # 한국 시장 데이터 로딩[cite: 1]
    ...
    def _build_industry_lookup(self, tickers):
        """산업 ETF 티커 리스트를 분석하여 종목별 산업 매핑 생성"""
        lookup = {}
...
# ---------------------------------------------------------
# 4. 페이지 처리 (자기참조 방지 및 교차 연결 허용)
# ---------------------------------------------------------
def process_page(page, engine, client, config):
    ...
    if clean_t in engine.kr_listing or clean_t in engine.kr_etf:
        ...
        try:
            ...
            logger.info(f"✅ {clean_t} (KR) 업데이트 완료")
        except Exception as e:
...
def main():
    # 타임아웃 60초 설정
    client = Client(auth=NOTION_TOKEN, request_timeout=60)
    
    # 🌟 지표 DB 동적 분석 (KR시장, KR산업 등)
    config = get_dynamic_config(client)
    
    # 한국 주식 전용 엔진 초기화
    engine = StockAutomationEngine(config["kr_industry_tickers"])
    ...
    if all_pages:
        with ThreadPoolExecutor(max_workers=5) as executor:
            for page in all_pages:
                executor.submit(process_page, page, engine, client, config)
                time.sleep(0.05)
