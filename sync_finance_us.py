...
# ==============================================================================
# 2. 해외 주식 재무 및 퀀트 지표 수집부 (Yahoo Finance & KIS)
# ==============================================================================
def get_stock_financials(
    ...
    # 배당수익률 포맷팅
    div_y = us_data.get("dividend_yield")
    if div_y is not None and div_y > 0.5:
        div_y = div_y / 100.0
    ...
    return {
        ...
        "현재가": curr_p,
        "PER": us_data.get("per"),
        "추정PER": us_data.get("forward_per"),
        "EPS": us_data.get("eps"),
        "추정EPS": us_data.get("forward_eps"),
        "PBR": us_data.get("pbr"),
        "BPS": us_data.get("bps"),
        "배당수익률": div_y,
        "52주 최고가": w52_h,
        "52주 최저가": w52_l,
        "목표주가": us_data.get("target_price"),
        "투자의견": us_data.get("opinion"),
        "직전고점": safe_float(quant.get("swing_high")),
        "직전저점": safe_float(quant.get("swing_low")),
        "200일선": safe_float(quant.get("ma200")) or us_data.get("ma_200"),
        "50일선": safe_float(quant.get("ma_supply")) or us_data.get("ma_50"),
        "수급선": safe_float(quant.get("ma_supply")) or us_data.get("ma_50"),
        "추세": quant.get("trend"),
        "스마트 가이드": quant.get("smart_guide"),
        "모멘텀 진단": quant.get("mom_diag"),
        "위험도 등급": quant.get("risk_grade"),
        "12M 모멘텀": safe_float(quant.get("mom_12m")),
        "52주 낙폭": safe_float(quant.get("drawdown_52w")),
        "낙폭율": safe_float(quant.get("drawdown_52w")),
        "60일 변동성": safe_float(quant.get("vol_60d")),
    }
...
def build_finance_update_for_page(
    ...
    number_keys = [
        "현재가", "PER", "추정PER", "EPS", "추정EPS", "PBR", "BPS", "배당수익률",
        "목표주가", "52주 최고가", "52주 최저가",
        "직전고점", "직전저점", "200일선", "50일선", "수급선", "12M 모멘텀", "52주 낙폭", "낙폭율", "60일 변동성"
    ]
    select_keys = ["추세", "스마트 가이드", "모멘텀 진단", "위험도 등급", "투자의견"]
...
# ==============================================================================
def main() -> None:
    """해외 주식 재무 및 퀀트 정보 일괄 업데이트 메인 파이프라인"""
    ...
    logger.info("🚀 [2단계] 해외 주식 재무/퀀트 지표 대량 동기화 시작 (Yahoo Finance)")
