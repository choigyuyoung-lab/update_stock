...
- **포트폴리오 위험도(VaR)**: 95% 신뢰수준 1주일 최대 예상 변동성 약 **-{portfolio_var_krw:,.0f}원 (-{portfolio_var_pct:.2f}%)** (가중 변동성: {portfolio_weighted_vol:.1f}%)
...
### 🎯 [스마트 밸류 에버리징 (Value Averaging) 주간 100만원 적립금 최적 배분 산출]
...
- **차주 전술적 매크로 대응 가이드**: (차주 이벤트, 7대 자산 퀀트 순위, 환율({fx_rate:,.1f}원) 대비 주간 행동 지침: 명사형 종결)

### 2. 🌟 K-올라운드 포트폴리오 총평 & 자산배분 건전성
- **올라운드 적합도 점수**: **XX점** / 100점 (판정: 🔴 리밸런싱 시급 / 🟡 주의 / 🟢 최적)
- **리스크 진단**: 95% 1주일 최대 예상 변동성 -{portfolio_var_krw:,.0f}원 (-{portfolio_var_pct:.2f}%) 수준으로 안정적/위험 관리 상태 (명사형 종결)
- **핵심 강점 2가지**:
  1. (강점 1: 명사형 종결)
  2. (강점 2: 명사형 종결)
- **핵심 취약점 2가지**:
...
|:---|:---:|:---|:---:|:---|:---|
*(삼성이전, 미래연금, 삼성ISA, 삼성IRP, 일반계좌별 추천 ETF 및 밸류 에버리징 추천금액 배분)*
* (표 하단 실행 가이드 2개 불릿: 명사형 종결)

### 6. ✅ 차주 실행 체크리스트 (3대 핵심 액션)
1. **스마트 밸류 에버리징 적립금 집행**: (상승추세 및 저평가 자산 우선 매수 플랜: 명사형 종결)
