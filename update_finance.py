...
def main():
    ...
    while True:
        ...
        for page in pages:
            ...
            eps, bps = (get_kr_fin(ticker)) if is_kr else (yf.Ticker(ticker).info.get("trailingEps"), yf.Ticker(ticker).info.get("bookValue"))
            
            try:
                upd = {}
                if is_valid(eps): upd["EPS"] = {"number": eps}
                if is_valid(bps): upd["BPS"] = {"number": bps}
                if upd: notion.pages.update(page_id=page["id"], properties=upd)
                print(f"   [{ticker}] 재무 완료")
            except: pass
            time.sleep(0.4)
