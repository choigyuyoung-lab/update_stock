...
# ---------------------------------------------------------
# 2. 지표 엔진 (FDR + YFinance 인메모리 메타데이터)
# ---------------------------------------------------------
...
def process_benchmark_page(page: Dict[str, Any], engine: BenchmarkAutomationEngine, client: Any) -> Optional[Dict[str, Any]]:
    pid = page["id"]
    ...
    return {
        ...
        "keywords": parse_keywords(kw_val, fallback_summary=summary)
    }

# ---------------------------------------------------------
# 4. 마스터 DB 정합성 진단 (헬스체크 & 커버리지 리포트)
# ---------------------------------------------------------
def run_master_db_health_check(client: Any, benchmark_list: List[Dict[str, Any]], engine: BenchmarkAutomationEngine):
