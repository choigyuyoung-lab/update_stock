...
class StockAutomationEngine:
    def __init__(self):
        logger.info(f"📡 엔진 시작 (수동 전체 업데이트 모드: {IS_FULL_UPDATE})") # 로그 추가로 확인 가능
        ...
        # 2. 우량주 맵 구축 (태그는 'KOSDAQ GLOBAL' 유지)
        self.blue_chip_map = {
    ...
    def _get_kglobal(self) -> List[str]:
        """KOSDAQ GLOBAL 태그용 리스트 (Market 컬럼 필터링)"""
    ...
    def get_stock_detail(self, clean_t: str) -> Dict[str, Any]:
        """Market 명칭 통일: KOSDAQ GLOBAL 등도 모두 'KOSDAQ'으로"""
        kr_match = self.df_krx[self.df_krx['Code'] == clean_t]
        if not kr_match.empty:
            row = kr_match.iloc[0]
            mkt = "KOSDAQ" if "KOSDAQ" in str(row['Market']) else str(row['Market']) # 통일 로직
